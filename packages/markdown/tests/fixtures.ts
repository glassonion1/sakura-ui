 ...
 * replaced. They are drawn from what the content of the GCAS guide actually
 * uses, measured over its 509 documents, rather than from what Markdown allows.
 */
...
export const plain = {
  ...
  gfmTable: '| 見出し | 値 |\n| --- | --- |\n| あ | 1 |\n| い | 2 |',
  codeFence: '```sh\necho "hello"\n```\n\n```\n言語指定なし\n```',
  links: '[内部](/foo) と [外部](https://example.com) と https://example.com/bare',
...
export const rawHtml = {
  // 637 pairs across 18 files, often with Markdown inside
  details:
    '<details>\n<summary>詳細はこちら</summary>\n\n- リスト項目\n\n| 表 | 見出し |\n| --- | --- |\n| a | b |\n\n</details>',
  // 30 of the 56 files with an HTML table need cells that span
  tableSpan:
  ...
  // style appears 1,733 times over 125 files
  styledBlock:
    '<div style="background-color: #E8F1FE; padding: 12px 16px; border-radius: 8px; margin: 0 24px;">\n注記のボックス\n</div>',
  styledImg: '<img src="/images/a.png" alt="図" style="width: 800px;color-scheme: light;">',
  inlineTags: '本文に <strong>太字</strong> と <u>下線</u> と <sup>（※1）</sup> と <small>小</small>。',
  externalAnchor: '<a href="https://example.com" target="_blank">外部リンク</a>'
}

export const malformedHtml = {
  // </br> appears 265 times over 14 files
  closingBr: '一行目<br>二行目</br>三行目</BR>四行目',
  // </td> mistyped as </d>, six times in one file
...
export const directives = {
  linkButton: 'ご案内です。:link-button[サービス一覧]{href=/services} をどうぞ。',
...
export const notDirectives = {
  ...
  chapter: '第1章:はじめに',
  // Taken from the GCAS guide, where these lose text today: "HH:MM" is written
  // 10 times, "chat:write" 6 times, and the link label 3 times.
